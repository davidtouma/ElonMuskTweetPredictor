...
if LOCAL == False:
   stub = modal.Stub()
   hopsworks_image = modal.Image.debian_slim().pip_install(["hopsworks", "tweepy"])
...
def g():
  ...
  feature_list = []
  for features in tweets_df.columns:
    feature_list.append(features)
  
  
  
  tweets_fg = fs.get_or_create_feature_group(
      name = "elon_musk_tweets_modal",
      version=1,
      primary_key = feature_list,
      description  = "Scraped tweets from Elon Musk")

  tweets_fg.insert(tweets_df, write_options={"wait_for_job" : False})
